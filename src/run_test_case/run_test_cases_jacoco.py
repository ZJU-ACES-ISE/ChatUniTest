...
# Dependencies
COBERTURA_DIR = "/root/TestGPT_ASE/src/run_test_case/dependencies/cobertura-2.1.1"
JUNIT_JAR = "/root/TestGPT_ASE/src/run_test_case/dependencies/lib/junit-platform-console-standalone-1.9.2.jar"
MOCKITO_JAR = "/root/TestGPT_ASE/src/run_test_case/dependencies/lib/mockito-core-3.12.4.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/mockito-inline-3.12.4.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/mockito-junit-jupiter-3.12.4.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/byte-buddy-1.14.4.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/byte-buddy-agent-1.14.4.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/objenesis-3.3.jar"
LOG4J_JAR = "/root/TestGPT_ASE/src/run_test_case/dependencies/lib/slf4j-api-1.7.5.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/slf4j-log4j12-1.7.12.jar:\
/root/TestGPT_ASE/src/run_test_case/dependencies/lib/log4j-1.2.17.jar"
JACOCO_AGENT = "/root/TestGPT_ASE/src/run_test_case/dependencies/jacoco/jacocoagent.jar"
JACOCO_CLI = "/root/TestGPT_ASE/src/run_test_case/dependencies/jacoco/jacococli.jar"
...
def instrument(project_path, instrument_dir, datafile_dir):
    ...
    result = subprocess.run(["bash", os.path.join(COBERTURA_DIR, "cobertura-instrument.sh"),
                             ...
                             f"{project_path}/target/classes"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def report(datafile_dir, report_dir, build_dir):
    # print("Reporting...", datafile_dir, report_dir)
    if not os.path.exists(report_dir):
        os.mkdir(report_dir)

    build_list = build_dir.split(":")
    classfiles = ""
    ...
    result = subprocess.run(["java", "-jar", JACOCO_CLI, "report", f"{datafile_dir}/jacoco.exec", classfiles,
                             "--csv", os.path.join(report_dir, "coverage.csv")], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def run_single_test(tests, test_case_file, dependencies, build_dir, compiled_tests_dir, compiler_output, test_output,
    ...
    try:
        ...
        # jacoco
        result = subprocess.run(
            ["java", f"-javaagent:{JACOCO_AGENT}=destfile={compiled_tests_dir}/jacoco.exec", f"@{classpath_file}",
             "org.junit.platform.console.ConsoleLauncher", "--disable-banner", "--disable-ansi-colors",
             "--fail-if-no-tests", "--details=none", "--select-class", full_test_name], timeout=TIMEOUT,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE, text=True)
...
def run_all_tests(project_path, tests_dir, build_dir, compiled_tests_dir, compiler_output, test_output, report_dir):
    ...
    for t in range(1, 1 + TEST_NUMBER):
        ...
        report(compiled_tests_dir, os.path.join(report_dir, str(t)), build_dir)
...
def start_single_test(test_case_src, target_dir):
    ...
    try:
        ...
        if not run_single_test(temp_dir, test_case, dependencies, build_dir,
            ...
            report(compiled_tests_dir, temp_dir, build_dir)
    except Exception as e:
        print(e)
        return False
    return True
...
if __name__ == "__main__":
    ...
    test_cases_src = '/root/TestGPT_ASE/result/scope_test%20230430172828%'
    target_dir = '/root/TestGPT_ASE/projects/chart'
